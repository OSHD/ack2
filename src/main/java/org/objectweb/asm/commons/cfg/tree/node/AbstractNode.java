...
public class AbstractNode extends Tree<AbstractNode> implements Opcodes {
        ...
        switch (insn.type()) {
            case INSN: {
                if (opcode() >= ICONST_M1 && opcode() <= DCONST_1) {
                    ...
                    nv.visitArrayLoad((ArrayLoadNode) this);
                } else {
                    nv.visit(this);
                }
                break;
    ...
    public AbstractNode first(final Filter<AbstractNode> filter) {
        for (final AbstractNode n : this) {
            if (filter.accepts(n))
                return n;
        }
        return null;
    ...
    public List<AbstractNode> findChildren(Class type) {
        List<AbstractNode> children = new ArrayList<>();
        for (AbstractNode n : this) {
            if (n.getClass() == type) children.add(n);
        }
        return !children.isEmpty() ? children : null;
    }

    public List<AbstractNode> layerAll(Class... types) {
        List<AbstractNode> children = findChildren(types[0]);
        if (children == null) return null;
        if (types.length == 1) return children;
        for (int i = 1; i < types.length; i++) {
            List<AbstractNode> next = new ArrayList<>();
            for (AbstractNode n : children) {
                List<AbstractNode> match = n.findChildren(types[i]);
                if (match == null) continue;
                next.addAll(match);
            }
            if (next.isEmpty()) {
    ...
    public AbstractNode layer(Class... types) {
        List<AbstractNode> nodes = layerAll(types);
    ...
    public AbstractNode preLayer(Class... types) {
        AbstractNode node = this;
        for (Class type : types) {
            node = node.parent();
            if (node == null || node.getClass() != type) return null;
        }
        return node;
    }
