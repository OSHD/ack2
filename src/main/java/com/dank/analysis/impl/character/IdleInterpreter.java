 ...
 */
public class IdleInterpreter extends FieldInterpreter {

    private String decoder;
    private int iterValue = 0;

    public IdleInterpreter(String decoder) {
        super(decoder);
        this.decoder = decoder;
    }

    String[] names = new String[]{"npcBoundDim", "npcDegToTurn", "walkAnimation", "npcTurnAround", "npcTurnRight", "npcTurnLeft", "idleAnimation", "getNextAnimation", "unknown7"};
    ...
    public BasicValue binaryOperation(final AbstractInsnNode insn, final BasicValue value1, final BasicValue value2)
        ...
        if (insn.opcode() == Opcodes.PUTFIELD) { // REF VALUE
            if (value2 instanceof DeocdedField) { // This operation is using a interesting value...
                ...
                if (put_field.owner.equals(Hook.NPC.getInternalName())) {  // Character.?? = NPCDEF.?? * LDC

                    put_field.owner = put_field.owner.replaceAll("[a-z]+", Hook.CHARACTER.getInternalName());
//                    addFieldContainer(new RSField(names[iterValue], put_field));
                    Hook.CHARACTER.put(new RSField(names[iterValue], put_field));
                    iterValue++;
                }
            }
