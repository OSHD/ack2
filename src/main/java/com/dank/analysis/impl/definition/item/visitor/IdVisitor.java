package com.dank.analysis.impl.definition.item.visitor;
...
public class IdVisitor extends TreeVisitor {
    ...
    public void visitConversion(final ConversionNode cn) {
        if (cn.fromInt() && cn.toLong()) {
            FieldMemberNode fmn = (FieldMemberNode) cn.layer(IADD, IADD, ISHL, IMUL, GETFIELD);
            if (fmn == null) fmn = (FieldMemberNode) cn.layer(IMUL, GETFIELD);
            if (fmn != null) {
                Hook.ITEM_DEFINITION.put(new RSField(fmn, "id"));
            }
        }
    }
}
